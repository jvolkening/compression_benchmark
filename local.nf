...
process collect_data {
    ...
    output:
    file "*.il.fq" into test_files mode flatten
    file "${row.accession}.meta.tmp" into meta_files

    script:
    """
    clumpify.pl \
        --in ${baseDir}/raw/${row.accession}_R1_001.fastq.gz \
        --in2 ${baseDir}/raw/${row.accession}_R2_001.fastq.gz \
        --out clumped_1.fq.gz \
